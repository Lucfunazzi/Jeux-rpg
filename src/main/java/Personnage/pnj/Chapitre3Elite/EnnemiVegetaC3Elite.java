...
public class EnnemiVegetaC3Elite extends PersonnageBase {

    public EnnemiVegetaC3Elite() { this(48); }

    public EnnemiVegetaC3Elite(int niveau) {
        this.nom    = "Vegeta";
        this.niveau = niveau;
        this.type   = "Guerrier";
        this.role   = "DPS";
    ...
    @Override public void attaqueUltime(List<PersonnageBase> equipeAlliee, List<PersonnageBase> equipeEnnemie, List<String> log) {
        log.add("Vegeta — Final Flash devastateur !");
        for (PersonnageBase c : equipeEnnemie)
            if (c.estVivant()) {
                Combat.appliquerDegatsAvecLog(this, c, this.getAttaque() * 1.60, log);
                Combat.appliquerEffet(this, c, new ReductionDefense(0.25, 3), log);
