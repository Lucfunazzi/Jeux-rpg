...
public class Chapitre9Elite implements ChapitreElite {
    ...
    private final Chapitre9      chapitre9;
    private final Chapitre8Elite chapitre8Elite;

    public Chapitre9Elite(Chapitre9 chapitre9, Chapitre8Elite chapitre8Elite) {
        this.chapitre9      = chapitre9;
        this.chapitre8Elite = chapitre8Elite;
        stagesDebloques[1]    = true;
    }

    // -- Condition de deblocage --------------------------------------------
    public boolean estDebloque() {
        return chapitre9.getStagesReussis()[10]
            && chapitre8Elite.getStagesReussis()[10];
    }

    public void afficher(GameContext ctx, Scanner scanner) {
        if (!estDebloque()) {
            System.out.println("Terminez le Chapitre 9 et le Chapitre 8 Elite pour debloquer le Chapitre 9 Elite !");
            return;
        }
        ...
        while (!retour) {
            ...
            System.out.println("\n========================================");
            System.out.println("   CHAPITRE 9 ELITE -- " + getNomChapitre());
            System.out.println("========================================");
            ...
            for (int i = 1; i <= NB_STAGES; i++) {
                boolean jouable = ctx.gestionnaireQuetes.estStageJouable(9, i, true);
                String etat     = !stagesDebloques[i] ? "[###] "
                        : stagesReussis[i]             ? "[OK]  "
                        : jouable                       ? "[  ]  "
                        :                                 "[QUETE] ";
                int    restants = ctx.gestionnaireEnergie.getRunsEliteRestants(i);
                String etoiles  = ctx.gestionnaireEtoiles.getEtoiles(9, i, true).afficher();
            ...
            } else if (!ctx.gestionnaireQuetes.estStageJouable(9, choix, true)) {
    ...
    public Stage.ResultatStage lancerStage(GameContext ctx, int numero) {
        ...
        if (resultatStage.victoire) {
            ...
            if (numero < NB_STAGES) {
                ...
                System.out.println(">> Felicitations ! Vous avez termine le Chapitre 9 Elite !");
                ctx.gestionnaireTitres.debloquerTitre("[Titre a definir] -- Chapitre 9 Elite");
            }

            ctx.gestionnaireQuetes.notifierOrGagne(stage.getRecompenseOr());
            ctx.gestionnaireQuetes.notifierStageFini(9, numero, true,
                    ctx.joueur, ctx.menuRecrutement, ctx.personnagesRecruites);
            ctx.gestionnaireEtoiles.mettreAJour(9, numero, true,
