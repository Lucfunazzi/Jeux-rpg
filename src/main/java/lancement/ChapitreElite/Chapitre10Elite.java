...
public class Chapitre10Elite implements ChapitreElite {
    ...
    private final Chapitre10      chapitre10;
    private final Chapitre9Elite chapitre9Elite;

    public Chapitre10Elite(Chapitre10 chapitre10, Chapitre9Elite chapitre9Elite) {
        this.chapitre10      = chapitre10;
        this.chapitre9Elite = chapitre9Elite;
        stagesDebloques[1]    = true;
    }

    // -- Condition de deblocage --------------------------------------------
    public boolean estDebloque() {
        return chapitre10.getStagesReussis()[10]
            && chapitre9Elite.getStagesReussis()[10];
    }

    public void afficher(GameContext ctx, Scanner scanner) {
        if (!estDebloque()) {
            System.out.println("Terminez le Chapitre 10 et le Chapitre 9 Elite pour debloquer le Chapitre 10 Elite !");
            return;
        }
        ...
        while (!retour) {
            ...
            System.out.println("\n========================================");
            System.out.println("   CHAPITRE 10 ELITE -- " + getNomChapitre());
            System.out.println("========================================");
            ...
            for (int i = 1; i <= NB_STAGES; i++) {
                boolean jouable = ctx.gestionnaireQuetes.estStageJouable(10, i, true);
                String etat     = !stagesDebloques[i] ? "[###] "
                        : stagesReussis[i]             ? "[OK]  "
                        : jouable                       ? "[  ]  "
                        :                                 "[QUETE] ";
                int    restants = ctx.gestionnaireEnergie.getRunsEliteRestants(i);
                String etoiles  = ctx.gestionnaireEtoiles.getEtoiles(10, i, true).afficher();
            ...
            } else if (!ctx.gestionnaireQuetes.estStageJouable(10, choix, true)) {
    ...
    public Stage.ResultatStage lancerStage(GameContext ctx, int numero) {
        ...
        if (resultatStage.victoire) {
            ...
            if (numero < NB_STAGES) {
                ...
                System.out.println(">> Felicitations ! Vous avez termine le Chapitre 10 Elite !");
                ctx.gestionnaireTitres.debloquerTitre("[Titre a definir] -- Chapitre 10 Elite");
            }

            ctx.gestionnaireQuetes.notifierOrGagne(stage.getRecompenseOr());
            ctx.gestionnaireQuetes.notifierStageFini(10, numero, true,
                    ctx.joueur, ctx.menuRecrutement, ctx.personnagesRecruites);
            ctx.gestionnaireEtoiles.mettreAJour(10, numero, true,
