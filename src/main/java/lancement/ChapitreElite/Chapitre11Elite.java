...
public class Chapitre11Elite implements ChapitreElite {
    ...
    private final Chapitre11      chapitre11;
    private final Chapitre10Elite chapitre10Elite;

    public Chapitre11Elite(Chapitre11 chapitre11, Chapitre10Elite chapitre10Elite) {
        this.chapitre11      = chapitre11;
        this.chapitre10Elite = chapitre10Elite;
        stagesDebloques[1]    = true;
    }

    // -- Condition de deblocage --------------------------------------------
    public boolean estDebloque() {
        return chapitre11.getStagesReussis()[10]
            && chapitre10Elite.getStagesReussis()[10];
    }

    public void afficher(GameContext ctx, Scanner scanner) {
        if (!estDebloque()) {
            System.out.println("Terminez le Chapitre 11 et le Chapitre 10 Elite pour debloquer le Chapitre 11 Elite !");
            return;
        }
        ...
        while (!retour) {
            ...
            System.out.println("\n========================================");
            System.out.println("   CHAPITRE 11 ELITE -- " + getNomChapitre());
            System.out.println("========================================");
            ...
            for (int i = 1; i <= NB_STAGES; i++) {
                boolean jouable = ctx.gestionnaireQuetes.estStageJouable(11, i, true);
                String etat     = !stagesDebloques[i] ? "[###] "
                        : stagesReussis[i]             ? "[OK]  "
                        : jouable                       ? "[  ]  "
                        :                                 "[QUETE] ";
                int    restants = ctx.gestionnaireEnergie.getRunsEliteRestants(i);
                String etoiles  = ctx.gestionnaireEtoiles.getEtoiles(11, i, true).afficher();
            ...
            } else if (!ctx.gestionnaireQuetes.estStageJouable(11, choix, true)) {
    ...
    public Stage.ResultatStage lancerStage(GameContext ctx, int numero) {
        ...
        if (resultatStage.victoire) {
            ...
            if (numero < NB_STAGES) {
                ...
                System.out.println(">> Felicitations ! Vous avez termine le Chapitre 11 Elite !");
                ctx.gestionnaireTitres.debloquerTitre("[Titre a definir] -- Chapitre 11 Elite");
            }

            ctx.gestionnaireQuetes.notifierOrGagne(stage.getRecompenseOr());
            ctx.gestionnaireQuetes.notifierStageFini(11, numero, true,
                    ctx.joueur, ctx.menuRecrutement, ctx.personnagesRecruites);
            ctx.gestionnaireEtoiles.mettreAJour(11, numero, true,
