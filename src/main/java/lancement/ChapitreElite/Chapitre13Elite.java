...
public class Chapitre13Elite implements ChapitreElite {
    ...
    private final Chapitre13      chapitre13;
    private final Chapitre12Elite chapitre12Elite;

    public Chapitre13Elite(Chapitre13 chapitre13, Chapitre12Elite chapitre12Elite) {
        this.chapitre13      = chapitre13;
        this.chapitre12Elite = chapitre12Elite;
        stagesDebloques[1]    = true;
    }

    // -- Condition de deblocage --------------------------------------------
    public boolean estDebloque() {
        return chapitre13.getStagesReussis()[10]
            && chapitre12Elite.getStagesReussis()[10];
    }

    public void afficher(GameContext ctx, Scanner scanner) {
        if (!estDebloque()) {
            System.out.println("Terminez le Chapitre 13 et le Chapitre 12 Elite pour debloquer le Chapitre 13 Elite !");
            return;
        }
        ...
        while (!retour) {
            ...
            System.out.println("\n========================================");
            System.out.println("   CHAPITRE 13 ELITE -- " + getNomChapitre());
            System.out.println("========================================");
            ...
            for (int i = 1; i <= NB_STAGES; i++) {
                boolean jouable = ctx.gestionnaireQuetes.estStageJouable(13, i, true);
                String etat     = !stagesDebloques[i] ? "[###] "
                        : stagesReussis[i]             ? "[OK]  "
                        : jouable                       ? "[  ]  "
                        :                                 "[QUETE] ";
                int    restants = ctx.gestionnaireEnergie.getRunsEliteRestants(i);
                String etoiles  = ctx.gestionnaireEtoiles.getEtoiles(13, i, true).afficher();
            ...
            } else if (!ctx.gestionnaireQuetes.estStageJouable(13, choix, true)) {
    ...
    public Stage.ResultatStage lancerStage(GameContext ctx, int numero) {
        ...
        if (resultatStage.victoire) {
            ...
            if (numero < NB_STAGES) {
                ...
                System.out.println(">> Felicitations ! Vous avez termine le Chapitre 13 Elite !");
                ctx.gestionnaireTitres.debloquerTitre("[Titre a definir] -- Chapitre 13 Elite");
            }

            ctx.gestionnaireQuetes.notifierOrGagne(stage.getRecompenseOr());
            ctx.gestionnaireQuetes.notifierStageFini(13, numero, true,
                    ctx.joueur, ctx.menuRecrutement, ctx.personnagesRecruites);
            ctx.gestionnaireEtoiles.mettreAJour(13, numero, true,
