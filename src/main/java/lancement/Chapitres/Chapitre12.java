...
public class Chapitre12 implements Chapitre {
    ...
    public void afficher(GameContext ctx, Scanner scanner) {
        ...
        while (!retour) {
            ...
            System.out.println("\n========================================");
            System.out.println("   CHAPITRE 12 — " + getNomChapitre());
            System.out.println("========================================");
            ...
            for (int i = 1; i <= NB_STAGES; i++) {
                boolean jouable = ctx.gestionnaireQuetes.estStageJouable(12, i, false);
                String etat = stagesReussis[i] ? "[OK]  "
                        : !stagesDebloques[i]  ? "[###] "
                        : jouable              ? "[  ]  "
                        :                        "[QUETE] ";
                String etoiles = ctx.gestionnaireEtoiles.getEtoiles(12, i, false).afficher();
            ...
            } else if (!ctx.gestionnaireQuetes.estStageJouable(12, choix, false)) {
    ...
    public Stage.ResultatStage lancerStage(GameContext ctx, int numero) {
        ...
        if (resultatStage.victoire) {
            ...
            if (numero < NB_STAGES) {
                ...
                System.out.println(">> Felicitations ! Chapitre 12 termine !");
            }

            ctx.gestionnaireQuetes.notifierOrGagne(stage.getRecompenseOr());
            ctx.gestionnaireQuetes.notifierStageFini(12, numero, false,
                    ctx.joueur, ctx.menuRecrutement, ctx.personnagesRecruites);
            ctx.gestionnaireEtoiles.mettreAJour(12, numero, false,
