...
public class Chapitre11 implements Chapitre {
    ...
    public void afficher(GameContext ctx, Scanner scanner) {
        ...
        while (!retour) {
            ...
            System.out.println("\n========================================");
            System.out.println("   CHAPITRE 11 — " + getNomChapitre());
            System.out.println("========================================");
            ...
            for (int i = 1; i <= NB_STAGES; i++) {
                boolean jouable = ctx.gestionnaireQuetes.estStageJouable(11, i, false);
                String etat = stagesReussis[i] ? "[OK]  "
                        : !stagesDebloques[i]  ? "[###] "
                        : jouable              ? "[  ]  "
                        :                        "[QUETE] ";
                String etoiles = ctx.gestionnaireEtoiles.getEtoiles(11, i, false).afficher();
            ...
            } else if (!ctx.gestionnaireQuetes.estStageJouable(11, choix, false)) {
    ...
    public Stage.ResultatStage lancerStage(GameContext ctx, int numero) {
        ...
        if (resultatStage.victoire) {
            ...
            if (numero < NB_STAGES) {
                ...
                System.out.println(">> Felicitations ! Chapitre 11 termine !");
            }

            ctx.gestionnaireQuetes.notifierOrGagne(stage.getRecompenseOr());
            ctx.gestionnaireQuetes.notifierStageFini(11, numero, false,
                    ctx.joueur, ctx.menuRecrutement, ctx.personnagesRecruites);
            ctx.gestionnaireEtoiles.mettreAJour(11, numero, false,
