...
/**
 * Gère la Créature Sacrée du joueur (univers Dragon Ball Z).
 *
 * Progression :
 *   - Débloqué au niveau 30 du joueur.
 *   - L'œuf est obtenu via quête (à appeler via debloquerOeuf()).
...
public class GestionnaireCreaturesSacrees {
    ...
    /** True si l'entraînement est en cours et le temps est écoulé. */
    ...
    private String ajouterXP(int xp, String source) {
        ...
        if (estAuNiveauMax()) {
            experience = experienceMax; // barre pleine au max
            sb.append(type.nom).append(" est au niveau maximum ! Évoluez-le !\n");
        }
    ...
    public String evoluer() {
        ...
        if (!type.peutEvoluer())    return type.nom + " est déjà la forme finale disponible.";
    ...
    public void afficher(int niveauJoueur) {
        ...
        if (!oeufDebloque) {
            ...
            System.out.println("║  Complétez une quête pour en recevoir un.║");
        ...
        if (entrainementEnCours()) {
            ...
            System.out.println("║  Créature au maximum actuel !");
            System.out.println("║  [0] Retour");
        } else {
